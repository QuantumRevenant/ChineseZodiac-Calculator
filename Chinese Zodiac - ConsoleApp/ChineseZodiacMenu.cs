...
class CZMenu
{
    public static void ChineseZodiacMenu()
    {
        do
        {
            int option;
            ...
            option = QRMenu.OptionMenu("Zodiaco de Año Chino", "¿Qué deseas hacer?", options,true,true);
    ...
    public static void DoSingleConsoleQuery(int year)
    {
        DoRangeConsoleQuery(year - 1, year + 1);
    }
    public static void DoRangeConsoleQuery(int year1, int year2)
    {
        PrintTitle();
    ...
    public static void DoRangeExcelQuery(int year1, int year2)
    {
        ...
        for (int year = year1; year <= year2; year++)
        {
            ...
            $"{output.zodiacSign.ZodiacAnimal,-10}," +                                       // Animal (ajustado a 10 caracteres)
            $"{output.Kua.number.male},{output.Kua.number.female}," +                                // Kua para hombre y mujer (2 dígitos)
            $"{output.zodiacSign.ZodiacElement,-8}," +                                       // Elemento animal (ajustado a 8 caracteres)
            $"{output.Kua.group.male,-9}," +                                                 // Coordenadas Kua hombre (ajustado a 9 caracteres)
            $"{output.Kua.group.female,-9}," +                                               // Coordenadas Kua mujer (ajustado a 9 caracteres)
            $"{output.zodiacSign.ZodiacPolarity,-4},"                                        // Polaridad animal (ajustado a 4 caracteres)
            );
        }
